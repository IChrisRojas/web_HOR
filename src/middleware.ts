...
export const onRequest = defineMiddleware(async (context, next) => {
    ...
    if (context.url.pathname.startsWith('/keystatic')) {
        ...
        // Retrieve credentials from environment variables (Cloudflare Secrets)
        const runtime = context.locals.runtime;
        const adminUser = runtime?.env?.ADMIN_USERNAME || import.meta.env.ADMIN_USERNAME;
        const adminPass = runtime?.env?.ADMIN_PASSWORD || import.meta.env.ADMIN_PASSWORD;
